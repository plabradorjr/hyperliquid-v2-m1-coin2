        ...
        if current_position:
            ...
            if position_side == "long" and not ignore_longs and not ignore_exit:
                if check_long_exit_condition(current_candle, previous_candle):
                    ...
                    # Immediately check long entry after closing the position
                    try:
                        ...
                        if not ignore_longs and check_long_entry_condition(current_candle, previous_candle):
                            ...
                            if not ignore_sl:
                                trailing = float(params.get("trailing_sl_pct", 0)) if not ignore_trailing_sl else 0
                                if trailing > 0:
                                    sl_price = compute_trailing_long_sl_level(current_price)
                                else:
                        ...
                        my_print(
                            f"Error during immediate long entry after long exit: {e}", verbose)
            ...
            # Check short entry
                ...
                if not ignore_sl:
                    trailing = float(params.get("trailing_sl_pct", 0)) if not ignore_trailing_sl else 0
                    if trailing > 0:
                        sl_price = compute_trailing_short_sl_level(current_price)
                    else:
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument("--debug-orders", action="store_true", help="log open orders and SL detection")
